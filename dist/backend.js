...
function normalizeLoreName(value) { return String(value || '').normalize('NFKC').trim().toLocaleLowerCase().replace(/[\s_-]+/g, ' '); }
function stableLoreValue(value) { if (Array.isArray(value)) return value.map(stableLoreValue); if (value && typeof value === 'object') return Object.fromEntries(Object.keys(value).sort().map(key => [key, stableLoreValue(value[key])])); return value; }
function loreEntrySignature(entry) { const copy={}; for(const [key,value] of Object.entries(entry||{})){ if(['id','uid','world_book_id','created_at','updated_at'].includes(key)) continue; copy[key]=stableLoreValue(value); } return JSON.stringify(stableLoreValue(copy)); }
async function listAllLoreBooks(userId){const all=[];let offset=0;while(true){const page=await spindle.world_books.list({limit:200,offset},userId);all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
async function listAllLoreEntries(worldBookId,userId){const all=[];let offset=0;while(true){const page=await spindle.world_books.entries.list(worldBookId,{limit:200,offset},userId);all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
async function listAllCharacters(userId){const all=[];let offset=0;while(true){const page=await spindle.characters.list({limit:200,offset},userId);all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
function loreSimilarity(a,b){const left=new Set(a),right=new Set(b),union=new Set([...left,...right]);if(!union.size)return 1;let common=0;for(const value of left)if(right.has(value))common+=1;return common/union.size;}
function recommendedLoreKeeper(books){return [...books].sort((a,b)=>{const ra=a.characterRefs+(a.globalRef?1:0),rb=b.characterRefs+(b.globalRef?1:0);if(ra!==rb)return rb-ra;if(a.entryCount!==b.entryCount)return b.entryCount-a.entryCount;return a.id.localeCompare(b.id);})[0].id;}
async function scanLorebookDuplicates(userId){const [books,characters,globalIds]=await Promise.all([listAllLoreBooks(userId),listAllCharacters(userId),spindle.world_books.getGlobal(userId)]);const charRefs=new Map();for(const character of characters)for(const id of character.world_book_ids||[])charRefs.set(id,(charRefs.get(id)||0)+1);const globalSet=new Set(globalIds),summaries=[];for(const book of books){const entries=await listAllLoreEntries(book.id,userId);summaries.push({id:book.id,name:book.name,entryCount:entries.length,characterRefs:charRefs.get(book.id)||0,globalRef:globalSet.has(book.id),signatures:entries.map(loreEntrySignature).sort()});}const groups=new Map();for(const summary of summaries){const key=normalizeLoreName(summary.name);if(!key)continue;const arr=groups.get(key)||[];arr.push(summary);groups.set(key,arr);}const results=[];for(const [normalized,items] of groups){if(items.length<2)continue;let min=1;for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++)min=Math.min(min,loreSimilarity(items[i].signatures,items[j].signatures));const exact=min===1&&items.every(x=>x.signatures.length===items[0].signatures.length);const classification=exact?'exact':min>=.75?'near':'conflicting',keep=recommendedLoreKeeper(items);results.push({groupId:encodeURIComponent(normalized),name:items[0].name,classification,similarity:min,recommendedKeepId:keep,books:items.map(x=>({id:x.id,name:x.name,entryCount:x.entryCount,characterRefs:x.characterRefs,globalRef:x.globalRef}))});}return results.sort((a,b)=>a.name.localeCompare(b.name));}
async function rewireLorebookReferences(keepId,duplicateIds,userId){const dup=new Set(duplicateIds),characters=await listAllCharacters(userId);let updated=0;for(const character of characters){const current=Array.isArray(character.world_book_ids)?character.world_book_ids:[];if(!current.some(id=>dup.has(id)))continue;const next=Array.from(new Set(current.map(id=>dup.has(id)?keepId:id)));await spindle.characters.update(character.id,{world_book_ids:next},userId);updated+=1;}const globals=await spindle.world_books.getGlobal(userId);if(globals.some(id=>dup.has(id)))await spindle.world_books.setGlobal(Array.from(new Set(globals.map(id=>dup.has(id)?keepId:id))),userId);return updated;}
async function mergeLorebookEntries(keepId,duplicateIds,userId){const signatures=new Set((await listAllLoreEntries(keepId,userId)).map(loreEntrySignature));let created=0;for(const duplicateId of duplicateIds)for(const entry of await listAllLoreEntries(duplicateId,userId)){const sig=loreEntrySignature(entry);if(signatures.has(sig))continue;const input={...entry};for(const k of ['id','uid','world_book_id','created_at','updated_at'])delete input[k];await spindle.world_books.entries.create(keepId,input,userId);signatures.add(sig);created+=1;}return created;}
async function applyLorebookCleanupGroup(group,merge,userId){const keepId=typeof group?.recommendedKeepId==='string'?group.recommendedKeepId:'';const duplicateIds=Array.isArray(group?.books)?group.books.map(book=>String(book?.id||'')).filter(id=>id&&id!==keepId):[];if(!keepId||!duplicateIds.length)throw new Error('Invalid cleanup group.');const mergedEntries=merge?await mergeLorebookEntries(keepId,duplicateIds,userId):0,updatedCharacters=await rewireLorebookReferences(keepId,duplicateIds,userId);let deletedBooks=0;for(const id of duplicateIds)if(await spindle.world_books.delete(id,userId))deletedBooks+=1;return{mergedEntries,updatedCharacters,deletedBooks};}
...
spindle.onFrontendMessage(async (payload, userId) => {
    ...
    if (payload?.type === 'lorebook_cleanup_scan') {
        ...
        catch (error) { spindle.sendToFrontend({ type: 'lorebook_cleanup_action_result', ok: false, error: error?.message || String(error) }, userId); }
        return;
    }
