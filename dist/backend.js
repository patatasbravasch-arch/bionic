...
async function listAllLoreBooks(){const all=[];let offset=0;while(true){const page=await spindle.world_books.list({limit:200,offset});all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
async function listAllLoreEntries(worldBookId){const all=[];let offset=0;while(true){const page=await spindle.world_books.entries.list(worldBookId,{limit:200,offset});all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
async function listAllCharacters(){const all=[];let offset=0;while(true){const page=await spindle.characters.list({limit:200,offset});all.push(...page.data);if(all.length>=page.total||!page.data.length)return all;offset+=page.data.length;}}
...
async function scanLorebookDuplicates(){const [books,characters,globalIds]=await Promise.all([listAllLoreBooks(),listAllCharacters(),spindle.world_books.getGlobal()]);const charRefs=new Map();for(const character of characters)for(const id of character.world_book_ids||[])charRefs.set(id,(charRefs.get(id)||0)+1);const globalSet=new Set(globalIds),summaries=[];for(const book of books){const entries=await listAllLoreEntries(book.id);summaries.push({id:book.id,name:book.name,entryCount:entries.length,characterRefs:charRefs.get(book.id)||0,globalRef:globalSet.has(book.id),signatures:entries.map(loreEntrySignature).sort()});}const groups=new Map();for(const summary of summaries){const key=normalizeLoreName(summary.name);if(!key)continue;const arr=groups.get(key)||[];arr.push(summary);groups.set(key,arr);}const results=[];for(const [normalized,items] of groups){if(items.length<2)continue;let min=1;for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++)min=Math.min(min,loreSimilarity(items[i].signatures,items[j].signatures));const exact=min===1&&items.every(x=>x.signatures.length===items[0].signatures.length);const classification=exact?'exact':min>=.75?'near':'conflicting',keep=recommendedLoreKeeper(items);results.push({groupId:encodeURIComponent(normalized),name:items[0].name,classification,similarity:min,recommendedKeepId:keep,books:items.map(x=>({id:x.id,name:x.name,entryCount:x.entryCount,characterRefs:x.characterRefs,globalRef:x.globalRef}))});}return results.sort((a,b)=>a.name.localeCompare(b.name));}
async function rewireLorebookReferences(keepId,duplicateIds){const dup=new Set(duplicateIds),characters=await listAllCharacters();let updated=0;for(const character of characters){const current=Array.isArray(character.world_book_ids)?character.world_book_ids:[];if(!current.some(id=>dup.has(id)))continue;const next=Array.from(new Set(current.map(id=>dup.has(id)?keepId:id)));await spindle.characters.update(character.id,{world_book_ids:next});updated+=1;}const globals=await spindle.world_books.getGlobal();if(globals.some(id=>dup.has(id)))await spindle.world_books.setGlobal(Array.from(new Set(globals.map(id=>dup.has(id)?keepId:id))));return updated;}
async function mergeLorebookEntries(keepId,duplicateIds){const signatures=new Set((await listAllLoreEntries(keepId)).map(loreEntrySignature));let created=0;for(const duplicateId of duplicateIds)for(const entry of await listAllLoreEntries(duplicateId)){const sig=loreEntrySignature(entry);if(signatures.has(sig))continue;const input={...entry};for(const k of ['id','uid','world_book_id','created_at','updated_at'])delete input[k];await spindle.world_books.entries.create(keepId,input);signatures.add(sig);created+=1;}return created;}
async function applyLorebookCleanupGroup(group,merge){const keepId=typeof group?.recommendedKeepId==='string'?group.recommendedKeepId:'';const duplicateIds=Array.isArray(group?.books)?group.books.map(book=>String(book?.id||'')).filter(id=>id&&id!==keepId):[];if(!keepId||!duplicateIds.length)throw new Error('Invalid cleanup group.');const mergedEntries=merge?await mergeLorebookEntries(keepId,duplicateIds):0,updatedCharacters=await rewireLorebookReferences(keepId,duplicateIds);let deletedBooks=0;for(const id of duplicateIds)if(await spindle.world_books.delete(id))deletedBooks+=1;return{mergedEntries,updatedCharacters,deletedBooks};}
