...
function transformPlainText(text: string): string {
  return text.replace(WORD_RE, (word) => {
    ...
    if (cut <= 0 || cut >= units.length) return word
...
function readHtmlTag(source: string, start: number): HtmlTag | null {
  ...
  if (cursor === nameStart || !/[A-Za-z]/.test(source[nameStart])) return null
...
function findRawHtmlElementEnd(source: string, start: number): number | null {
  const opening = readHtmlTag(source, start)
  if (!opening) return null
  if (!opening.name || opening.closing || opening.selfClosing) return opening.end
...
function findFenceBlockEnd(source: string, start: number): number | null {
  ...
  while (cursor < source.length) {
    ...
    if (indent <= 3 && source[p] === marker) {
      ...
      if (run >= markerLength) {
        let rest = p + run
        while (source[rest] === ' ' || source[rest] === '\t') rest++
        if (source[rest] === '\n') return rest + 1
        if (rest >= source.length) return source.length
      }
    }
...
function findClosingBracket(source: string, start: number, open: string, close: string): number | null {
  let depth = 0
...
function findImageTokenEnd(source: string, start: number): number | null {
...
function findUrlEnd(source: string, start: number): number | null {
 ...
 */
export function transformMarkdownForBionic(source: string): string {
  if (!source || source.includes('class="lumibionic-fix"')) return source
