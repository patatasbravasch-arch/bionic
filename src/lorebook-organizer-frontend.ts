...
export function installLorebookOrganizer(
  ...
  */
  ...
  function similarNameGroups() {
    ...
    /*
      This deliberately compares names only.

      characterMatchKey() already removes common export/copy
      noise such as:
      - (1), copy, duplicate
      - lorebook / worldbook / world info
      - sillytavern
      - NSFW / SFW
      - version suffixes
      - separator/punctuation differences

      Contents are NOT compared here.
    */
    for (const book of books) {
      const key =
        characterMatchKey(
    ...
    for (
      const [key, groupedBooks]
      of byName.entries()
    ) {
      if (
        groupedBooks.length < 2
      ) {
        continue
      }
      ...
      /*
        Use the shortest real name as the group heading.
        This normally favors the clean/base filename.
      */
    ...
    /*
      Most useful case first:
      one copy is linked while another similarly named copy
      has no references.
    */
    result.sort((a, b) => {
      if (a.mixed !== b.mixed) {
  ...
  async function replaceSimilarReferences(
    keeperId: string
  ) {
    if (busy) return
    ...
    if (!group) {
      renderAll(
        'That similar-name group no longer exists. Rescan and try again.'
      )
      return
    }
    ...
    if (!affectedRefs.length) {
      renderAll(
        'The other similar-name lorebooks have no references to replace.'
      )
      return
    }
    ...
    const summary = [
      affectedCharacterIds.size
        ? `${affectedCharacterIds.size} character${affectedCharacterIds.size === 1 ? '' : 's'}`
        : '',
      affectedChatIds.size
        ? `${affectedChatIds.size} chat${affectedChatIds.size === 1 ? '' : 's'}`
        : '',
      affectedPersonaIds.size
        ? `${affectedPersonaIds.size} persona${affectedPersonaIds.size === 1 ? '' : 's'}`
        : '',
      affectsGlobal
        ? 'global activation'
        : '',
    ...
    renderAll(
      `Refreshing references before replacing with "${keeper.name}"…`
    )
    ...
    try {
      ...
      if (!group) {
        throw new Error(
          'The similar-name group changed during verification.'
        )
      }
      ...
      /*
        Characters:
        remove every old member of this similar-name group,
        preserve unrelated books, then attach the keeper.
      */
      for (
      ...
      /*
        Chats:
        preserve unrelated chat lorebooks and replace only
        members of this similar-name group.
      */
      for (
        const chatId of
          affectedChatIds
        ...
        const freshChat =
          await api(
            `/api/v1/chats/${encodeURIComponent(chatId)}?messages=false`
          )
        ...
        await api(
          `/api/v1/chats/${encodeURIComponent(chatId)}/metadata`,
          {
            method: 'PATCH',
            body:
              JSON.stringify({
      ...
      /*
        Personas have a single lorebook attachment.
      */
      if (
        affectedPersonaIds.size
      ) {
      ...
      /*
        Global activation:
        replace old group members with the selected keeper.
      */
      if (affectsGlobal) {
        const currentGlobalIds =
          Array.isArray(
            snapshot?.globalIds
          )
            ? snapshot.globalIds
      ...
      /*
        Refresh and verify that none of the old copies retain
        references.
      */
      await freshReferences()

      const remainingReferencedOldBooks =
        books.filter(
          book =>
            oldIds.has(book.id) &&
            book.references.length > 0
      ...
      if (
        remainingReferencedOldBooks.length
      ) {
        throw new Error(
          `${remainingReferencedOldBooks.length} old similar-name lorebook${remainingReferencedOldBooks.length === 1 ? '' : 's'} still have references after replacement.`
        )
      }

      resultMessage =
        `Replaced references with "${keeper.name}". The old copies are now left in the library for review/deletion.`
    } catch (error: any) {
      resultMessage =
        `Replacement stopped: ${
          error?.message ||
  ...
  async function unifySimilarCharacters(
    keeperId: string
  ) {
    if (busy) return
    ...
    if (!initialCharacterIds.size) {
      renderAll(
        'No character links were found in this similar-name group.'
      )
      return
    }
    ...
    renderAll(
      `Refreshing references before consolidating characters onto "${keeper.name}"…`
    )
    ...
    try {
      /*
        Refresh immediately before mutation. Similar-name matches
        are only a UI grouping; character references remain the
        authoritative source for this operation.
      */
      await freshReferences()
      ...
      if (!refreshedKeeper) {
        throw new Error(
          'The selected lorebook no longer exists.'
        )
      }
      ...
      if (!characterIds.length) {
        throw new Error(
          'No character references remain in this group.'
        )
      }
      ...
      for (
        ...
        /*
          Preserve every unrelated lorebook attachment.
          Only IDs belonging to this Similar Names group are
          collapsed onto the selected keeper.
        */
        const nextIds =
          Array.from(
            new Set([
        ...
        if (
          ...
          throw new Error(
            `Character link verification failed for ${characterId}.`
          )
        }
        ...
        if (
          ...
          throw new Error(
            `Character ${characterId} still contains another lorebook from this similar-name group.`
          )
        }
        ...
        renderAll(
          `Linked ${updated}/${characterIds.length} character${characterIds.length === 1 ? '' : 's'} to "${refreshedKeeper.name}"…`
        )
      }
      ...
      resultMessage =
        `Linked ${characterIds.length} character${characterIds.length === 1 ? '' : 's'} to "${refreshedKeeper.name}". Other similar lorebooks were not deleted; chat, persona and global links were left unchanged.`
    } catch (error: any) {
      resultMessage =
        `Character consolidation stopped: ${
          error?.message ||
          String(error)
        }`
    } finally {
        ...
        visible.length
          ...
          : `
            <div class="lb-organizer-empty">
              ${
                allGroups.length
                  ? 'No similar-name groups match this search.'
                  : books.length
                    ? 'No similar lorebook names found.'
                    : 'Scan the library to begin.'
              }
            </div>
          `
    ...
    modalRoot.addEventListener(
      ...
      event => {
        ...
        const replaceSimilarButton =
          target.closest(
            '[data-organizer-similar-replace-with]'
          ) as HTMLElement | null

        if (replaceSimilarButton) {
          const keeperId =
            replaceSimilarButton.dataset
              .organizerSimilarReplaceWith ||
            ''

          if (keeperId) {
            void replaceSimilarReferences(
              keeperId
            )
          }
        ...
        const unifyCharactersButton =
          target.closest(
            '[data-organizer-similar-unify-characters]'
          ) as HTMLElement | null

        if (unifyCharactersButton) {
          const keeperId =
            unifyCharactersButton.dataset
              .organizerSimilarUnifyCharacters ||
            ''

          if (keeperId) {
            void unifySimilarCharacters(
              keeperId
            )
          }
